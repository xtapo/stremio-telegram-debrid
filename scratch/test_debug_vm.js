...
async function decryptEmbed(embedUrl) {
    ...
    class CustomBlob {
        constructor(parts, options) {
            ...
            console.log("[Blob created] length:", this.size);
        }
        async text() {
    ...
    const mockJwPlayerInstance = {
        setup: function(cfg) {
            console.log("[mockJwPlayer setup called] cfg:", cfg);
    ...
    const mockJwPlayer = function(id) {
        console.log("[mockJwPlayer called] id:", id);
        return mockJwPlayerInstance;
    };
...
decryptEmbed("https://embed18.streamc.xyz/embed.php?hash=c9e5230c3e65847df88fc05ea66cbbb6")
    .then(res => {
        console.log("SUCCESS!");
        console.log("m3u8 snippet:", res.m3u8.slice(0, 300));
    })
    .catch(err => {
        console.error("Error in decryptEmbed:", err);
    });
